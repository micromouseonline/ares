 ...
 */
Window::Window() { m_window.create(conf::VideoMode, "Window from constructor"); }
...
void Window::Update() {
  ...
  while (m_window.pollEvent(event)) {
    if (event.type == sf::Event::Closed) { m_isDone = true; } else if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::F5) {
      ToggleFullscreen();
    } else {
...
void Window::AddObserver(IEventObserver* observer) { m_observers.push_back(observer); }

void Window::NotifyObservers(const Event& event) { for (auto& observer : m_observers) { observer->OnEvent(event); } }
